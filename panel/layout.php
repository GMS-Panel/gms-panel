...
// Kullanim: layout_head('Sayfa Basligi', 'aktif-menu');
//           layout_foot();
...
function layout_head(string $title = 'GMS Panel', string $active = 'dashboard') {
    ...
    $admin_menu = [
        ...
        'users'     => ['icon'=>'ti-user-shield',      'label'=>'Kullanicilar','href'=>'users.php'],
    ...
    $user_menu = [
        ...
        'ssl'       => ['icon'=>'ti-lock',             'label'=>'SSL',        'href'=>'ssl.php'],
        'backup'    => ['icon'=>'ti-device-floppy',    'label'=>'Yedekleme',  'href'=>'backup.php'],
        'php'       => ['icon'=>'ti-brand-php',        'label'=>'PHP Ayarlari','href'=>'php_settings.php'],
        'files'     => ['icon'=>'ti-folder',           'label'=>'Dosyalar',   'href'=>'files.php'],
    ];

    $menu = $is_admin ? $admin_menu : $user_menu;
    $role_label = $is_admin ? 'Yonetici' : 'Kullanici';
    $role_color = $is_admin ? '#1d4ed8' : '#059669';
?>
<!DOCTYPE html>
<html lang="tr">
<head>
...
.brand-icon{width:34px;height:34px;background:<?= $role_color ?>;border-radius:8px;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:15px;color:#fff;flex-shrink:0}
...
.nav-section{font-size:10px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;padding:8px 10px 4px;margin-top:4px}
...
.user-avatar{width:30px;height:30px;border-radius:50%;background:<?= $role_color ?>22;border:1px solid <?= $role_color ?>44;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:<?= $role_color ?>;flex-shrink:0}
...
.alert-box{border-radius:var(--radius);padding:11px 14px;font-size:13px;display:flex;align-items:center;gap:10px;margin-bottom:20px}
...
.alert-warning{background:var(--amberbg);border:1px solid rgba(245,158,11,.3);color:var(--amber)}
</style>
</head>
<body>
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <div style="margin-top:8px;border-top:1px solid var(--border);padding-top:8px">
      <a href="/phpmyadmin" target="_blank" class="nav-item">
  ...
  <div class="sidebar-bottom">
    <div class="nav-user">
      <div class="user-avatar"><?= strtoupper(substr($_SESSION['gms_user'] ?? 'A', 0, 2)) ?></div>
      <div>
        <div class="user-name"><?= htmlspecialchars($_SESSION['gms_user'] ?? '') ?></div>
        <div class="user-role"><?= $role_label ?></div>
      </div>
    </div>
    <a href="logout.php" class="logout-btn"><i class="ti ti-logout"></i> Cikis Yap</a>
  </div>
</aside>
<div class="main">
  <div class="topbar">
    ...
    <div class="topbar-right">
      ...
      <span class="badge badge-green" style="font-size:11px">
        ...
        <?= htmlspecialchars($_SESSION['gms_hesap'] ?? '') ?>
      </span>
      <?php endif; ?>
